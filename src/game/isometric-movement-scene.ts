...
export class IsometricMovementScene extends Phaser.Scene {
	...
	create() {
		...
		this.cameras.main.centerOn(
			this.origin.x,
			this.origin.y + worldHeight * 0.35,
		);
	...
	stopAllCharacterMovement() {
		for (const character of this.characters.values()) {
			...
			const position = this.gridToScreen(
				character.cell.col,
				character.cell.row,
			);
			character.sprite.setPosition(position.x, position.y);
	...
	private registerCharacterTextures(data: MovementSceneData | undefined) {
		...
		for (const character of data.characters) {
			for (const [rotation, sprite] of character.sprites) {
				const key = this.characterTextureKey(
					character.id,
					rotation as TileRotation,
				);
				if (!this.textures.exists(key)) {
					this.textures.addCanvas(key, sprite.canvas);
				}
	...
	private assetScreenPosition(
		tile: Pick<PlacedTile, "col" | "row">,
		sprite: BakedPlaceableSprite,
	) {
		const centerCell = this.footprintCenter(
			tile.col,
			tile.row,
			sprite.footprint,
		);
	...
	private placeAssetImage(
		...
		return image
			...
			.setOrigin(
				sprite.originX / sprite.canvas.width,
				sprite.originY / sprite.canvas.height,
			)
			.setScale(scale)
	...
	private occupiedCellKeys(data: MovementSceneData | undefined) {
		return data
			? occupiedCellKeys(this.placedTiles, data.placeableSprites)
			: new Set<string>();
	}

	private blockedMovementCellKeys(data: MovementSceneData | undefined) {
		return data
			? blockedMovementCellKeys(this.placedTiles, data.placeableSprites)
			: new Set();
	}
	...
	private createCharacters() {
		...
		data.characters.forEach((characterConfig, index) => {
			...
			const cell = this.firstAvailableCharacterCell(
				data,
				index,
				reserved,
				characterConfig.cell,
			);
			reserved.add(cellKey(cell));
			const position = this.gridToScreen(cell.col, cell.row);
			const scale =
				(TILE_WIDTH / data.placeableSprites.diamondPx) * PLAYER_SCALE;
	...
	private firstAvailableCharacterCell(
		...
		if (
			!blocked.has(cellKey(PLAYER_START_CELL)) &&
			!reserved.has(cellKey(PLAYER_START_CELL))
		) {
	...
	private moveCharacterToAdjacentCell(
		...
		if (character.tween?.isPlaying()) {
			return Promise.resolve({
				success: false,
				reason: "Character is already moving.",
			} as const);
		}
		...
		if (
			...
			return Promise.resolve({
				success: false,
				reason: "Next path step is blocked.",
			} as const);
		}

		this.setCharacterRotation(
			character,
			this.directionToRotation(deltaCol, deltaRow),
		);
	...
	private updateCharacterDepth(character: SceneCharacter) {
		character.sprite.setDepth(
			ASSET_DEPTH_BASE + character.cell.col + character.cell.row + 1,
		);
	}

	private setCharacterRotation(
		character: SceneCharacter,
		rotation: TileRotation,
	) {
		...
		const data = this.registry.get("movementSceneData") as
			| MovementSceneData
			| undefined;
		const characterConfig = data?.characters.find(
	...
	private drawPlacementPreview(cell: { col: number; row: number } | null) {
		...
		for (const previewCell of preview.cells) {
			graphics.fillPoints(
				this.cellCorners(previewCell.col, previewCell.row),
				true,
				true,
			);
		}
	...
	private drawPlacementPreviewAsset(
		...
		const image =
			this.placementPreviewAsset ?? this.add.image(0, 0, preview.textureKey);
		...
		this.placeAssetImage(
			...
			.setAlpha(
				preview.isValid
					? VALID_ASSET_PREVIEW_ALPHA
					: INVALID_ASSET_PREVIEW_ALPHA,
			);
	...
	private cellCorners(col: number, row: number) {
		...
		const bottom = new Phaser.Math.Vector2(
			center.x,
