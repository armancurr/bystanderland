...
function projectedBoundsForBox(
	box: THREE.Box3,
	camera: THREE.Camera,
	canvasSize: number,
) {
...
function cropRenderedCanvas(
	...
	const cropRight = Math.min(
		source.width - 1,
		Math.ceil(bounds.maxX) + CROP_PADDING_PX,
	);
	const cropBottom = Math.min(
		source.height - 1,
		Math.ceil(bounds.maxY) + CROP_PADDING_PX,
	);
	...
	if (!ctx) {
		throw new Error(
			"Failed to acquire 2D context for cropped placeable sprite.",
		);
	}
...
function renderCroppedModel(
	...
	const origin = projectedCanvasPoint(
...
function getEngine(): BakeEngine {
	...
	created.loader = new GLTFLoader(
		createLoadingManager(() => created.currentPack),
	);
