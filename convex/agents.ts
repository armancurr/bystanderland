...
function fallbackMoveDecision(context: TurnContext): z.infer<typeof decisionSchema> {
...
async function enqueueValidatedDecision(
  ...
  if (
    (decision.actionId === "move_to_place" || decision.actionId === "inspect_place") &&
    (!decision.targetPlaceStableId || !placeExists(context, decision.targetPlaceStableId))
  ) {
...
async function runAgentTurnHelper(ctx: ActionCtx, characterId: string): Promise<AgentTurnResult> {
  const context: TurnContext | null = await ctx.runQuery(api.town.getTurnContext, { characterId });
  if (!context) {
  ...
  const tools = {
    moveToPlace: tool({
      ...
      inputSchema: z.object({
        targetPlaceStableId: z.string().describe("One of the place stableIds from the context."),
        task: z.string().nullable().describe("Short task label for the character."),
    ...
    moveToCell: tool({
      description: "Move this character to a specific grid cell when no labelled place fits.",
      inputSchema: z.object({
        col: z.number(),
        row: z.number(),
        task: z.string().nullable(),
    ...
    wait: tool({
      description: "Wait only if there is no useful movement or speech action available.",
      inputSchema: z.object({
        task: z.string().nullable(),
        reason: z.string(),
      }),
...
export const runPopulationStep = action({
  ...
  handler: async (ctx): Promise<PopulationStepResult> => {
    ...
    return {
      ok: okCount > 0,
      reason: okCount > 0 ? undefined : "No character produced a successful tool call.",
      results,
    };
  },
});
