...
async function reconcilePlaces(ctx: MutationCtx, worldId: Id<"worlds">) {
  ...
  const homeTilesByOwner = new Map(
    tiles
      .filter((tile) => tile.placeKind === "home" && tile.ownerCharacterId !== null)
  ...
  for (const character of characters) {
    ...
    await ctx.db.patch(character._id, { homePlaceStableId: stableId, updatedAt: timestamp });
  }
}
...
export const importLocalTiles = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const homeCandidates = args.tiles.filter(
      (tile) => tile.assetId.startsWith("suburban:") || tile.assetId.includes("building"),
    );
    ...
    await ctx.db.patch(world._id, { importedLocalStorage: true, updatedAt: timestamp });
...
export const getState = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      chatMessages: chatMessages.sort((a, b) => a.createdAt - b.createdAt).slice(-40),
    };
  },
});
...
export const enqueueAction = mutation({
  args: {
    ...
    targetCell: v.union(v.object({ col: v.number(), row: v.number() }), v.null()),
...
export const setActionStatus = mutation({
  args: {
    ...
    characterCell: v.union(v.object({ col: v.number(), row: v.number() }), v.null()),
  },
  handler: async (ctx, args) => {
    ...
    if (args.status === "completed" && action.actionId === "say" && action.message) {
